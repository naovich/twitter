...
export const initialState = {
  ...
  currentUser: {
    ...
    subscriptions: database.users[userId].subscriptions,
    subscribers: database.users[userId].subscribers,
  },
  userProfil: {
    ...
    subscriptions: database.users[userId].subscription,
    subscribers: database.users[userId].subscribers,
    tweets: [],
    tweetsUser: [],
  },
...
const reducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case "profil":
      return produce(state, (draft) => {
        ...
        draft.userProfil = {
          ...
          subscriptions: database.users[action.payload].subscriptions,
          subscribers: database.users[action.payload].subscribers,
    ...
    case "follow":
      return produce(state, (draft) => {
        draft.currentUser.subscriptions.push(action.payload);
      });

    case "unfollow":
      return produce(state, (draft) => {
        const target = draft.currentUser.subscriptions.findIndex(
          (element) => element === action.payload
        );
        draft.currentUser.subscriptions.splice(target, 1);
      });
    default:
      return {};
  }
